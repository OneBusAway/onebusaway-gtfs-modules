...
public class DirectedGraph<T> {
  ...
  public DirectedGraph(DirectedGraph<T> graph) {
    for (T node : graph.getNodes()) addNode(node);
    for (Pair<T> edge : graph.getEdges()) addEdge(edge.getFirst(), edge.getSecond());
  }
  ...
  private void removeNode(T node) {
    ...
    for (T from : get(_inboundEdges, node, false)) get(_outboundEdges, from, false).remove(node);
    _inboundEdges.remove(node);

    for (T to : get(_outboundEdges, node, false)) get(_inboundEdges, to, false).remove(node);
  ...
  public List<T> getTopologicalSort(Comparator<T> tieBreaker) {
    ...
    while (true) {
      ...
      if (noInbound.isEmpty()) throw new IllegalStateException("cycle");
  ...
  private boolean isConnected(T from, T to, Set<T> visited) {
    ...
    for (T next : get(_outboundEdges, from, false)) {
      if (visited.add(next)) {
        if (isConnected(next, to, visited)) return true;
      }
    }
