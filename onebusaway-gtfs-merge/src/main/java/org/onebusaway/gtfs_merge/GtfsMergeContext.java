...
/**
 * Manages state for the current merge operation, which is specific to
 * particular feed and GTFS entity type.
 * 
 * @author bdferris
 */
public class GtfsMergeContext {
  ...
  /**
   * @return a unique prefix that can be used to make duplicate or overlapping
   *         ids unique during the current merge operation.
   */
  ...
  /**
   * Indicate that an entity with the specified raw GTFS id as been added to the
   * merged output GTFS feed. Since typically only a single entity can be added
   * to a feed for a particular id, this method can be used in combination with
   * {@link #getEntityForRawId(String)} to detect id and entity collisions
   * across feeds.
   * 
   * @param rawId
   * @param entity
  ...
  /**
   * Find an entity registered with {@link #putEntityWithRawId(String, Object)}
   * for the current entity type across feeds merged so far, including the
   * current feed. Can be used to detect id and entity collisions across feeds.
   * 
   * @param rawId
   * @return an entity associated with the specified raw GTFS id for the current
   *         entity type, or null if no entity with the specified id exists.
   */
   ...
   * {@link #getEntityForRawId(String)} was added from the current source feed
   * or from some previous feed.
   * 
   * @param rawId
   * @return true if an entity with the specified id was added from the feed
   *         currently being processed with a call to
   *         {@link #putEntityWithRawId(String, Object)}.
   */
  ...
  /**
   * @return the duplicate detection strategy that has been chosen for the
   *         current entity type.
   */
  ...
  /**
   * Set the duplicate detection strategy that has been chosen for this entity
   * type. A strategy might have been specified directly by the user or we might
   * have auto-detected the best strategy based on available data.
   * 
