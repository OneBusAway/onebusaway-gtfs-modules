...
/**
 * In {@lint ValueSetter} implementations, the source value is often a primitive string but the
 * target value type is often something more complex: a numeric type, a {@link AgencyAndId}, a field
 * with GTFS-specific formatting, or even a GTFS entity. Often, the value cannot be properly
 * converted until the moment of assignment, as it must be resolved against existing and previously
 * assigned values.
 *
 * <p>This class provides methods for converting the input value into a resolved value appropriate
 * for assignment.
 */
public class DeferredValueConverter {
  ...
  public DeferredValueConverter(
      GtfsReader reader, EntitySchemaCache schemaCache, GtfsRelationalDao dao) {
    _support = new DeferredValueSupport(reader, schemaCache);
    _dao = dao;
  }

  /**
   * Converts the specified value as appropriate such that the resulting value can be assigned to
   * the specified property of the specified bean.
   */
  public Object convertValue(BeanWrapper targetBean, String targetPropertyName, Object value) {
    ...
    /**
     * When we introspect the "id" property of an IdentityBean instance, the return type is always
     * Serializable, when the actual type is String or AgencyAndId. This causes trouble with the
     * "isAssignableFrom" check below, so we do a first check here.
     */
    ...
    if (actualValueType == String.class) {
      ...
      Converter converter =
          _support.resolveConverter(parentEntityType, targetPropertyName, expectedValueType);
      if (converter != null) {
    ...
    throw new IllegalStateException(
        "no conversion possible from type \""
  ...
  private boolean isPrimitiveAssignable(Class<?> expectedValueType, Class<?> actualValueType) {
    ...
    return expectedValueType == Double.TYPE
        ...
        || expectedValueType == Long.TYPE
            && (actualValueType == Long.class
                || actualValueType == Integer.class
                || actualValueType == Short.class)
