...
public class TransformFactory {
  ...
  public void addModificationsFromFile(File path)
      throws IOException, TransformSpecificationException {
    BufferedReader reader = new BufferedReader(new FileReader(path));
    addModificationsFromReader(reader);
  }

  public void addModificationsFromString(String value)
      throws IOException, TransformSpecificationException {
  ...
  public void addModificationsFromReader(BufferedReader reader)
    ...
    while ((line = reader.readLine()) != null) {
      ...
      try {
        ...
        if (opType.equals("add")) {
        ...
        } else if (opType.equals(ARG_UPDATE)
            || opType.equals("change")
          ...
          handleTransformOperation(
              line, json, new RemoveNonRevenueStopsExcludingTerminalsStrategy());
          ...
          configureStopNameUpdates(
              _transformer, baseUrl + "/KingCountyMetroStopNameModifications.md");

          try {
            GtfsTransformerLibrary.configureTransformation(
                _transformer, baseUrl + "/KingCountyMetroModifications.md");
          ...
          throw new TransformSpecificationException(
              "unknown transform op \"" + opType + "\"", line);
        }
  ...
  private void handleAddOperation(String line, JSONObject json)
    ...
    if (entityType == null) {
      throw new TransformSpecificationMissingArgumentException(
          line, new String[] {ARG_CLASS, ARG_FILE}, ARG_OBJ);
    }
    Map<String, ValueSetter> propertyUpdates =
        getPropertyValueSettersFromJsonObject(entityType, objectSpec, _excludeForObjectSpec);
  ...
  private void handleUpdateOperation(String line, JSONObject json)
    ...
    if (json.has("strings")) {
      ...
      Map<String, Pair<String>> replacements =
          getEntityPropertiesAndStringReplacementsFromJsonObject(match.getType(), strings);
  ...
  private EntityTransformStrategy getUpdateEntityTransformStrategy(
      String line, TypedEntityMatch match, JSONObject update)
      throws JSONException, TransformSpecificationException {
    if (ServiceIdKey.class.isAssignableFrom(match.getType())) {
      ...
      Map<String, ValueSetter> propertyUpdates =
          getPropertyValueSettersFromJsonObject(match.getType(), update, emptySet);
  ...
  private void handleRetainOperation(String line, JSONObject json)
    ...
    if (json.has("retainUp")) retainUp = json.getBoolean("retainUp");
  ...
  private void handleTrimOperation(String line, JSONObject json)
    ...
    if (operation.getToStopId() == null && operation.getFromStopId() == null) {
      throw new TransformSpecificationMissingArgumentException(
          line, new String[] {"to_stop_id", "from_stop_id"});
    }
  ...
  private void setObjectPropertiesFromJsonUsingCsvFields(
      Object object, JSONObject json, String line)
    ...
    for (FieldMapping mapping : schema.getFields()) {
      try {
        ...
        String verboseMessage =
            "line=" + line + ", context=" + context + ", json=" + json + ", object=" + object;
  ...
  private TypedEntityMatch getMatch(String line, JSONObject json)
    ...
    if (entityType == null) {
      throw new TransformSpecificationMissingArgumentException(
          line, new String[] {ARG_FILE, ARG_CLASS}, ARG_MATCH);
    }
    ...
    for (Map.Entry<String, DeferredValueMatcher> entry : propertyMatches.entrySet()) {
      ...
      if (m.matches()) {
        PropertyPathCollectionExpression expression =
            new PropertyPathCollectionExpression(m.group(1));
  ...
  private TypedEntityMatch getCollectionMatch(String line, String collectionType, JSONObject match)
    ...
    if (collectionType.equals(ARG_CALENDAR)) {
      ...
      return new TypedEntityMatch(
          ServiceIdKey.class, new ServiceIdKeyMatch(_transformer.getReader(), serviceId));
      ...
      return new TypedEntityMatch(
          ShapeIdKey.class, new ShapeIdKeyMatch(_transformer.getReader(), shapeId));
    } else {
      throw new TransformSpecificationException(
          "unknown collection type: \"" + collectionType + "\"", line);
    }
  }
  ...
  private Map<String, ValueSetter> getPropertyValueSettersFromJsonObject(
    ...
    for (Map.Entry<String, Object> entry : map.entrySet()) {
      ...
      SingleFieldMapping mapping =
          _schemaCache.getFieldMappingForCsvFieldName(entityType, propertyName);
      if (mapping != null) {
  ...
  private ValueSetter createSetterForValue(Object value) {
    ...
    if (pathMatcher.matches()) {
      ...
      return new PropertyPathExpressionValueSetter(
