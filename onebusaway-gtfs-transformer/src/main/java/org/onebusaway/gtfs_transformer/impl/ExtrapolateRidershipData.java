...
public class ExtrapolateRidershipData implements GtfsTransformStrategy {
  ...
  public void run(TransformContext context, GtfsMutableRelationalDao dao) {
    ...
    // String is the TripId, ridershipData is the data from the Access DB where it is a list of all
    // the entries for that TripId, it should
    ...
    for (HashMap.Entry<String, TreeMap<Integer, String>> trip : ridershipTrips.entrySet()) {
      ...
      // for each ridership trip, we now have a tree list of the stops on that trip.  Compare to
      // each dao trip
      ...
      for (HashMap.Entry<AgencyAndId, TreeMap<Integer, String>> daoTrip : daoTrips.entrySet()) {
        ...
        if (equal) {
          ...
          String GTFStime =
              StopTimeFieldMappingFactory.getSecondsAsString(firstStopTime.getArrivalTime());
          // a lot of stop times are 'invalid' hours of 24, 25, 26.  For 24 hour clock, hour s/b
          // 0-23
          ...
          if (minutesDifference < 4 && minutesDifference > -4) {
            // _log.error("GTFS trip {}, time {}, Ridership trip {}, time {}, diff: {} ",
            // trip.getKey().getId(), GTFSArrival, ridershipStops.getKey(),passTime,
            // minutesDifference);
            ...
            // this is the list of data where the Trip ids are all the same, and each entry is a
            // stop
            ...
            // Do we need to handle when two ridership stops match to one GTFS? Compare and take the
            // one that is less?
    ...
    // the algorithm above takes each ridership trip and tries to find a GTFS trip to match it to.
    // The algorithm below
    // takes each GTFS trip and tries to find a ridership trip to match it to.  The numbers of
    // matched trips is the same either way.
