...
/**
 * using control file re-map GTFS stop ids and other stop properties from reference file The first
 * field in the control file ldrtifny is the reference stop id. For subway the stops from ATIS have
 * these ids, for example: 36, 9997, 31998 the stops in reference have these ids, for example: 138N,
 * 217N, 242S
 */
  ...
  public void run(TransformContext context, GtfsMutableRelationalDao dao) {
    ...
    for (String controlLine : controlLines) {
      ...
      if (direction.isEmpty()) {
        ...
        if (refStop == null) {
          if (!atisId.equals("0")) {
            // _log.info("missing reference stop {} for agency {} for ATIS id {}", referenceId,
            // getReferenceAgencyId(reference), atisId);
          }
          unmatched++;
          continue;
        }
      ...
      if (stopsUpdated.containsKey(refStop.getId())) {
        ...
        // _log.info("Stop times for stop: {} stopTimes: {}", atisStop.getId().getId(),
        // dao.getStopTimesForStop(atisStop).size());
        for (StopTime stopTime : dao.getStopTimesForStop(atisStop)) {
          stopTime.setStop(persistStop);
        }
        // _log.info("Duplicate stops keep: {}  remove: {} ", persistStop.getId().getId(),
        // atisStop.getId().getId());
        continue;
      }

      if (atisStop == null) {
        if (!atisId.equals("0")) {
          // _log.info("missing atis stop {} for Reference id {}{}", atisId, referenceId,
          // direction);
        }
        unmatched++;
        continue;
        ...
        matched++;
        // _log.error("Updated stop: original ATIS id: {} Reference id: {} Id now: {}", atisId,
        // referenceId, atisStop.getId().getId());
      }
    }
    _log.info(
