/**
 * Copyright (C) 2021 Cambridge Systematics, Inc. Copyright (C) 2013 Kurt Raschke
 * <kurt@kurtraschke.com> Copyright (C) 2011 Brian Ferris <bdferris@onebusaway.org>
 *
...
/**
 * Delete all stop times (arrival and departure) that are not timepoints and re-interpolate those
 * times with second precision.
 *
 * <p>For GTFS that has stops close together but also has minute precision on arrival/departure
 * times. These stops travel distance over no time which can cause issues with downstream systems.
 *
 * <p>Inspired by and ported from onebusaway-application-modules StopTimeEntriesFactory.
 */
public class InterpolateStopTimesFromTimePointsStrategy implements GtfsTransformStrategy {

  private final Logger _log =
      LoggerFactory.getLogger(InterpolateStopTimesFromTimePointsStrategy.class);
  ...
  private void interpolateArrivalAndDepartureTimes(
      GtfsMutableRelationalDao dao, Map<String, List<Trip>> tripsByBlockId) {
    for (List<Trip> trips : tripsByBlockId.values()) {
      for (Trip trip : trips) {
        ...
        interpolateArrivalAndDepartureTimes(
            dao, stopTimes, distanceTraveled, arrivalTimes, departureTimes);
  ...
  private void interpolateArrivalAndDepartureTimes(
    ...
    for (int i = 0; i < stopTimes.size(); i++) {
      ...
      if (hasDeparture && !hasArrival) {
        ...
        int t =
            departureTimes[i] =
                (int) InterpolationLibrary.interpolate(scheduleTimesByDistanceTraveled, d);
        arrivalTime = t;
        departureTime = t;
      }
      ...
      if (i > 0 && arrivalTimes[i] < departureTimes[i - 1]) {
        ...
        /**
         * The previous stop time's departure time comes AFTER this stop time's arrival time. That's
         * bad.
         */
        ...
        if (prevStop.equals(stop) && arrivalTimes[i] == departureTimes[i - 1] - 1) {
          _log.info(
              "fixing decreasing passingTimes: stopTimeA="
                  + prevStopTime.getId()
                  + " stopTimeB="
                  + stopTime.getId());
          arrivalTimes[i] = departureTimes[i - 1];
          if (departureTimes[i] < arrivalTimes[i]) departureTimes[i] = arrivalTimes[i];
          ...
          int t0 =
              (int)
                  InterpolationLibrary.interpolate(
          ...
          _log.warn(
              "correcting arrival time of sequence "
                  + (stopTime.getStopSequence() - 1)
                  + ", "
                  + stopTime.getStopSequence()
                  + " of trip "
                  + stopTime.getTrip().getId()
                  + " as it was less than last departure time.  Arrival["
                  + (i - 1)
                  + "] "
                  + arrivalTimes[i - 1]
                  + " now "
          ...
          for (int x = 0; x < stopTimes.size(); x++) {
            ...
            final String msg =
                x + " " + st.getId() + " " + arrivalTimes[x] + " " + departureTimes[x];
            _log.error(msg);
            System.err.println(msg);
          }
          final String exceptionMessage =
              "arrival time is less than previous departure time for stop time "
                  + " with isLenientArrivalDepartureTimes="
  ...
  /**
   * We have a list of StopTimes, along with their distance traveled along their trip/block. For any
   * StopTime that has either an arrival or a departure time, we add it to the SortedMaps of arrival
   * and departure times by distance traveled.
   *
   * @param stopTimes
   * @param distances
  ...
  private void populateArrivalAndDepartureTimesByDistanceTravelledForStopTimes(
    ...
    for (int i = 0; i < stopTimes.size(); i++) {
      ...
      /**
       * For StopTime's that have the same distance travelled, we keep the min arrival time and max
       * departure time
       */
  ...
  private void clearNonTimepointTimes(
      GtfsMutableRelationalDao dao, Map<String, List<Trip>> tripsByBlockId) {
    for (List<Trip> trips : tripsByBlockId.values()) {
      ...
      for (Trip trip : trips) {
        ...
        if (timepoints > 1) {
          ...
          for (StopTime stopTime : stopTimes) {
            ...
            // AND not the last stopTime
            if (0 == stopTime.getTimepoint() && count < stopTimes.size()) {
