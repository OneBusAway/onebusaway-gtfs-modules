...
public class RemoveRepeatedStopTimesStrategy implements GtfsTransformStrategy {
  ...
  public void run(TransformContext context, GtfsMutableRelationalDao dao) {
    ...
    Map<T2, List<Trip>> tripsByBlockId =
        TripsByBlockInSortedOrder.getTripsByBlockAndServiceIdInSortedOrder(dao);
          ...
          if (prev != null) {
            if (prev.getStop().getId().equals(stopTime.getStop().getId())) {
              ...
              stopTime.setDepartureTime(
                  Math.max(prev.getDepartureTime(), stopTime.getDepartureTime()));
