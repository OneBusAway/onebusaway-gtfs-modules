...
/**
 * Combines a serviceId (represented by {@link AgencyAndId}) and a {@link TimeZone} object that can
 * be used to ground {@link ServiceDate} objects. See {@link ServiceDate#getAsDate(TimeZone)} for
 * more info.
 *
 * @author bdferris
 * @see ServiceDate
...
public class LocalizedServiceId implements Serializable, Comparable<LocalizedServiceId> {
  ...
  public LocalizedServiceId(AgencyAndId serviceId, TimeZone timeZone) {
    if (serviceId == null) throw new IllegalArgumentException("serviceId cannot be null");
    if (timeZone == null) throw new IllegalArgumentException("timeZone cannot be null");
    this.id = serviceId;
  ...
  public int compareTo(LocalizedServiceId o) {
    ...
    if (rc == 0) rc = this.timeZone.getID().compareTo(o.getTimeZone().getID());
    return rc;
  }
}
