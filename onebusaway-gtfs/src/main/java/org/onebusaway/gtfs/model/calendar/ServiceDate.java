...
/**
 * A general representation of a year-month-day tuple not tied to any locale and used by the GTFS
 * entities {@link ServiceCalendar} and {@link ServiceCalendarDate} to represent service date
 * ranges. A service date is a particular date when a particular GTFS service id is active.
 *
 * @author bdferris
 */
public class ServiceDate implements Serializable, Comparable<ServiceDate> {
  ...
  public static ServiceDate parseString(String value) throws ParseException {
    ...
    if (!matcher.matches()) throw new ParseException("error parsing date: " + value, 0);
  ...
  /**
   * @return calls {@link #getAsDate(TimeZone)} with the default timezone for this VM. Be careful,
   *     this may return a time of 00:00:00 or 01:00:00 or 23:00:00 of the day before depending on
   *     EST or EDT
   */
   ...
   * ahead or behind. This behavior ensures correct calculation of {@link StopTime} arrival and
   * departure time when the second offset is added to the localized service date.
   *
  ...
  public long difference(ServiceDate serviceDate) {
    return (serviceDate.getAsDate(_utcTimeZone).getTime() - getAsDate(_utcTimeZone).getTime())
        / (24 * 60 * 60 * 1000);
  }
  ...
  public int compareTo(ServiceDate o) {
    ...
    if (c == 0) c = this.month - o.month;
    if (c == 0) c = this.day - o.day;
    return c;
  }
  ...
  /**
   * Adjust the supplied {@link Calendar} object such that the calendar will be at "midnight"
   * (12:00am) at the start of the day specified by the current calendar date and locale. Note that
   * we take the GTFS convention of calculating midnight by setting the target date to noon
   * (12:00pm) for the service date and timezone specified and then subtracting twelve hours.
   * Normally that would be equivalent to midnight, except on Daylight Saving Time days, in which
   * case it can be an hour ahead or behind. This behavior ensures correct calculation of {@link
   * StopTime} arrival and departure time when the second offset is added to the localized service
   * date.
   *
