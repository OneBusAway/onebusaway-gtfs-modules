...
public final class ServiceInterval implements Serializable {
  ...
  /**
   * Construct a new {@link ServiceInterval} by extending the current service interval, adjusting
   * the arrival and departure intervals to include the additional arrival and departure time
   * specified in the arguments.
   *
   * @param arrivalTime a new arrival time to incorporate in the extended interval
   * @param departureTime a new departure time to incorporate in the extended interval
   * @return a new interval with the additional arrival and departure times incorporated
   */
  ...
  public static ServiceInterval extend(
      ServiceInterval serviceInterval, int arrivalTime, int departureTime) {
    if (serviceInterval == null) return new ServiceInterval(arrivalTime, departureTime);
    else return serviceInterval.extend(arrivalTime, departureTime);
  }
