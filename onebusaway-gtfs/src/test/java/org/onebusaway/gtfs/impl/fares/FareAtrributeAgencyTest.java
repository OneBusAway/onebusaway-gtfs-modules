...
public class FareAtrributeAgencyTest {
  ...
  public void testAgenciesOnBartGtfs() throws Exception {
    ...
    final Set<String> agencyIdsInFareAttr =
        fareAttributes.stream().map(FareAttribute::getAgencyId).collect(Collectors.toSet());
    boolean isContainsBothExpectedAgencies =
        agencyIdsInFareAttr.contains("AirBART") && agencyIdsInFareAttr.contains("BART");
