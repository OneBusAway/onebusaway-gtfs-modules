...
public class CsvLineParserBenchmark {
  ...
  private final DelimitedTextParser delimitedTextParser = new DelimitedTextParser();
  private final LegacyDelimitedTextParser legacyDelimitedTextParser = new LegacyDelimitedTextParser(',');

  private final DelimitedTextParser skipWhitespaceDelimitedTextParser = new DelimitedTextParser();
  ...
  public CsvLineParserBenchmark() {
    skipWhitespaceDelimitedTextParser.setTrimInitialWhitespace(true);
  ...
  public int testParseStopTimes(ThreadState state) throws Exception {
    ...
    for(int i = 0; i < 1; i++) {
      for(String line : state.stopTimes) {
        count += delimitedTextParser.parse(line).size();
      }
      count++;
    }
    return count;
  ...
  @Benchmark
  public int testParseStopTimesSkipWhitespace(ThreadState state) throws Exception {
    int count = 0;
    for(int i = 0; i < 1; i++) {
      for(String line : state.stopTimes) {
        count += skipWhitespaceDelimitedTextParser.parse(line).size();
      }
      count++;
    }
    return count;
  ...
  @Benchmark
  public int testParseTripsSkipWhitespace(ThreadState state) throws Exception {
    int count = 0;
    for(int i = 0; i < 1; i++) {
      for(String line : state.trips) {
        count += skipWhitespaceDelimitedTextParser.parse(line).size();
