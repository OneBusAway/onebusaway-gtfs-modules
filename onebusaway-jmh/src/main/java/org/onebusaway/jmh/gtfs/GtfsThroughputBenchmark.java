...
@Fork(value = 10) // , jvmArgs= {"-XX:+UnlockDiagnosticVMOptions", "-Xlog:class+load=info",
// "-XX:+LogCompilation", "-XX:+DebugNonSafepoints"})
...
public class GtfsThroughputBenchmark {
  ...
  public GtfsRelationalDao testParse(ThreadState state) throws Exception {
    return processFeed(
        new File("./src/main/resources/island-transit_20090312_0314"), "abcd", false, state.reader);
  }
