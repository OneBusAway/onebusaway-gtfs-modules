 ...
 * #LegacyDefaultAgencyIdFieldMappingFactory(String)} constructor, we will evaluate that property
 * path expression against the target entity instance to determine the agencyId. So, for example, to
 * set the agencyId for {@link Route#getId()}, we specify a path of "agency.id", which will call
 * {@link Route#getAgency()} and then {@link Agency#getId()} to set the agency id. See also the path
 * "route.agency.id" for {@link Trip}.
 *
 * @author bdferris
 * @see GtfsEntitySchemaFactory
 */
public class LegacyDefaultAgencyIdFieldMappingFactory implements FieldMappingFactory {

  private String _agencyIdPath = null;

  public LegacyDefaultAgencyIdFieldMappingFactory() {
    this(null);
  }

  public LegacyDefaultAgencyIdFieldMappingFactory(String agencyIdPath) {
